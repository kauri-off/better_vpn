...
fn view(ctx: &mut Ctx) {
    ...
    let Some(info) = ui::report(ctx.call(|mut c| async move { c.get_cert_info(req).await })) else {
        return;
    };
...
fn generate(ctx: &mut Ctx) {
    ...
    let sans: Vec<String> = sans_s
        ...
        .map(str::to_string)
        .collect();
    let Ok(days_s) = ui::input_default("validity days", "3650") else {
        return;
    };
    ...
    let Ok(cert_path) = ui::input("cert path (blank = default)") else {
        return;
    };
    let Ok(key_path) = ui::input("key path (blank = default)") else {
        return;
    };

    if !matches!(
        ui::confirm("generate and overwrite the current cert?"),
        Ok(true)
    ) {
        return;
    }
